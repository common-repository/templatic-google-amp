<?php
/**
 * Plugin Name: Templatic Google AMP
 * Description: This plugin adds support for the Accelerated Mobile Pages (AMP) Project, which aims to provide mobile optimized content that can load instantly.
 * Author: Templatic
...
if(TEMPLATIC_AMP_ENABLE==1)
{
	...
	$end_point=explode('/',$_SERVER['REQUEST_URI']);
	/* Filter blank element from array */
	$end_point=array_filter($end_point);
	/* Check amp at last in url if it's exist rander AMP pages */
	if(end($end_point)==TEMPLATIC_AMP_ENDPOINT){
